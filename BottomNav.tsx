...
import { Home, Search, Grid, Heart, User, BookOpen } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activePage, onNavigate, onOpenAuth }) => {
  ...
  const navItems = [
    ...
    {
      id: 'profile',
      label: user ? (t('nav.profile') || 'Profil') : (t('auth.login') || 'Kirish'),
      icon: User,
      action: () => {
        if (user) {
          onNavigate('profile');
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-xl border-t border-purple-900/40 px-2 py-1.5 flex items-center justify-around shadow-2xl">
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-[10px] mt-0.5 tracking-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-[76px] text-center">
              {item.label}
            </span>
            {isActive && (
