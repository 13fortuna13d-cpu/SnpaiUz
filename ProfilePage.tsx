...
export const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  if (!user) {
    return (
      <div className="py-20 text-center space-y-4 max-w-md mx-auto bg-slate-900/60 p-8 rounded-3xl border border-slate-800">
        ...
        <p className="text-xs text-slate-400">{t('profile.login_required_desc')}</p>
  ...
  const handleCustomFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        alert(t('profile.error_avatar_size'));
        return;
      }
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <SeoHead title={`Profil - ${user.name} (AniSenpaiUz)`} />
      ...
      <div className="bg-gradient-to-r from-purple-950/60 via-slate-900 to-slate-900 rounded-3xl border border-purple-900/40 p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
          ...
          <div className="flex flex-col items-center sm:items-start gap-2.5 shrink-0">
            ...
            >
              ...
              <span>{user.avatar ? t('profile.change_photo_btn') : t('profile.add_photo_btn')}</span>
            </button>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2 justify-center sm:justify-start">
              ...
              {user.isVip ? (
                <span className="px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 font-black text-[10px] flex items-center gap-1 shadow-md">
                  <Crown className="w-3.5 h-3.5 text-amber-400" /> {t('profile.vip_badge')}
                </span>
              ) : (
                <span className="px-3 py-1 rounded-full bg-slate-800 text-slate-300 font-bold text-[10px]">
                  {t('profile.regular_badge')}
                </span>
              )}
            </div>
            ...
            <p className="text-slate-500 text-[11px]">
              {t('profile.registered_on')} {user.createdAt ? new Date(user.createdAt).toLocaleDateString('uz-UZ', { year: 'numeric', month: 'long', day: 'numeric' }) : new Date().toLocaleDateString('uz-UZ')}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap sm:flex-col gap-3 shrink-0 w-full sm:w-auto">
          ...
          >
            ...
            <span>{t('profile.topup_balance')}</span>
          </button>
          ...
          >
            ...
            <span>{t('profile.support_btn')} ({socialSettings.telegramUsername || '@SenpaiUzz'})</span>
          </a>
          ...
          {(user.role === 'admin' || user.role === 'super_admin') && (
            ...
            >
              ...
              <span>{t('profile.admin_panel_btn')}</span>
            </button>
          )}
        ...
        <div className="p-5 bg-slate-900/95 backdrop-blur-2xl rounded-3xl border border-purple-500/40 space-y-4 shadow-2xl animate-in fade-in">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-sm font-extrabold text-white flex items-center gap-2">
              ...
              <span>{t('profile.change_avatar_title')}</span>
            </h3>
          ...
          <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div>
              <p className="text-xs font-bold text-white">📱 {t('profile.upload_own_photo')}</p>
              <p className="text-[11px] text-slate-400">{t('profile.upload_hint')}</p>
            </div>
            <input
              type="file"
              ref={fileInputRef}
            ...
            >
              ...
              <span>{t('profile.choose_file')}</span>
            </button>
          </div>

          {/* Preset Anime Avatars */}
          <div className="space-y-2">
            <p className="text-xs font-bold text-slate-300">{t('profile.or_choose_preset')}</p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.balance_stat')}</span>
                <p className="text-base font-black text-emerald-400">{user.balanceUZS.toLocaleString()}</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.favorites_stat')}</span>
                <p className="text-base font-black text-pink-400">{favoritesCount} ta</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.watched_anime_stat')}</span>
                <p className="text-base font-black text-purple-400">{uniqueWatchedAnimeCount} ta</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.watched_episodes_stat')}</span>
                <p className="text-base font-black text-cyan-400">{totalWatchedEpisodes} qism</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.created_date_stat')}</span>
                <p className="text-xs font-bold text-slate-300">{createdDateFormatted}</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.last_active_stat')}</span>
                <p className="text-xs font-bold text-emerald-300">{lastActiveFormatted}</p>
              </div>

              <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 block font-medium">{t('profile.user_id_stat')}</span>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-800 pb-3">
        ...
        >
          ...
          <span>{t('profile.balance_and_history')}</span>
        </button>
        ...
        >
          ...
          <span>{t('profile.coins_and_referral')} ({user.coins || 0})</span>
        </button>
        ...
        >
          ...
          <span>{t('profile.favorites_tab')} ({favoriteAnimeList.length})</span>
        </button>
        ...
        >
          ...
          <span>{t('profile.history_tab')}</span>
        </button>
        ...
        <div className="bg-slate-900/80 rounded-3xl border border-slate-800 p-6 sm:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            ...
            >
              ...
              <span>{isEditingProfile ? t('common.cancel') : t('profile.edit_btn')}</span>
            </button>
          </div>
          ...
          {!isEditingProfile ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-slate-500 block">{t('profile.username_label')}</span>
                <span className="font-bold text-white text-sm">{user.name}</span>
              </div>

              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-slate-500 block">{t('profile.user_id_label')}</span>
                <span className="font-mono text-purple-300 text-sm font-bold">{user.id}</span>
              </div>

              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-slate-500 block">{t('profile.phone_label')}</span>
                <span className="font-mono text-white text-sm">{user.phone || t('profile.not_entered')}</span>
              </div>

              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-slate-500 block">{t('profile.email_label')}</span>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 block mb-1">{t('profile.name_username_label')}</label>
                <input
                  type="text"
                  required
                  value={editName}
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 block mb-1">{t('profile.email_field')}</label>
                <input
                  type="email"
                  required
                  value={editEmail}
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 block mb-1">{t('profile.phone_field')}</label>
                <input
                  type="text"
                  value={editPhone}
          ...
          <div className="p-6 bg-gradient-to-r from-amber-950/40 via-purple-950/40 to-slate-950 rounded-3xl border border-amber-500/30 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-black text-amber-300 text-base flex items-center gap-2">
                  ...
                  <span>{t('profile.vip_plans_title')}</span>
                </h4>
                <p className="text-xs text-slate-400">{t('profile.buy_direct_from_balance')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-center">
                ...
                <span className="text-lg font-black text-amber-400 block">15,000 so'm</span>
                <button
                  onClick={() => handleBuyVip(1, 15000)}
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-purple-500/40 relative space-y-2 text-center">
                <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full bg-purple-600 text-white text-[9px] font-black uppercase">{t('profile.popular_badge')}</span>
                <span className="text-xs font-bold text-slate-300 block">3 Oylik VIP</span>
                <span className="text-lg font-black text-amber-400 block">35,000 so'm</span>
                <button
                  onClick={() => handleBuyVip(3, 35000)}
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-center">
                ...
                <span className="text-lg font-black text-amber-400 block">150,000 so'm</span>
                <button
                  onClick={() => handleBuyVip(12, 150000)}
                  className="w-full py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs"
                >
                  {t('vip.subscribe_now')}
                </button>
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div>
              <h3 className="font-extrabold text-white text-lg flex items-center gap-2">
                ...
                <span>{t('profile.account_balance_title')}</span>
              </h3>
              <p className="text-xs text-slate-400">{t('profile.current_balance_label')} <strong className="text-emerald-400">{user.balanceUZS.toLocaleString()} so'm</strong></p>
            </div>
            ...
            >
              <Plus className="w-4 h-4" />
              <span>{t('profile.topup_short')}</span>
            </button>
          </div>

          {/* History table */}
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('profile.tx_history_label')}</h4>

            {user.balanceHistory.length === 0 ? (
              <p className="py-8 text-center text-slate-500 text-xs">{t('profile.no_tx_yet')}</p>
            ) : (
              <div className="space-y-2">
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-amber-950/60 via-slate-900 to-slate-900 border border-amber-500/40 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
            <div className="flex items-center gap-4 text-center sm:text-left">
              ...
              <div>
                <span className="text-xs font-bold text-amber-300/80 uppercase tracking-wider block">{t('profile.coin_balance_title')}</span>
                <h3 className="text-3xl font-black text-white flex items-center gap-2">
                  <span>{user.coins || 0}</span>
                  <span className="text-sm font-bold text-amber-400">{t('profile.coins_word')}</span>
                </h3>
                <p className="text-xs text-slate-400 mt-1">{t('profile.coins_usage_hint')}</p>
              </div>
            </div>
            ...
            >
              <Coins className="w-4 h-4" />
              <span>{t('profile.topup_coin_balance')}</span>
            </button>
          </div>
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border border-purple-500/30 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                <h3 className="text-lg font-black text-white flex items-center gap-2">
                  ...
                  <span>{t('profile.referral_system_title')}</span>
                </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 font-medium">{t('profile.your_referral_code')}</span>
                <p className="text-lg font-mono font-black text-purple-300">{user.referralCode || `SNP-${user.id}`}</p>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 font-medium">{t('profile.invited_friends')}</span>
                <p className="text-lg font-black text-emerald-400">{user.totalReferrals || 0} kishi</p>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <span className="text-[11px] text-slate-400 font-medium">{t('profile.earned_referral_bonus')}</span>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
              <label className="text-xs font-bold text-slate-300 block">{t('profile.your_referral_link')}</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={`https://anisenpaiuz.com/#ref=${user.referralCode || `SNP-${user.id}`}`}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`https://anisenpaiuz.com/#ref=${user.referralCode || `SNP-${user.id}`}`);
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 space-y-4 shadow-2xl">
            <h3 className="text-base font-extrabold text-white flex items-center gap-2">
              ...
              <span>{t('profile.coin_tx_history')}</span>
            </h3>

            {(!user.coinHistory || user.coinHistory.length === 0) ? (
              <p className="py-8 text-center text-slate-500 text-xs">{t('profile.no_coin_tx_yet')}</p>
            ) : (
              <div className="space-y-2">
      ...
      {activeTab === 'history' && (
        <div className="space-y-3">
          {user.watchHistory.length === 0 ? (
            <p className="text-center py-12 text-slate-500 text-sm">{t('profile.no_watch_history')}</p>
          ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="pt-6 border-t border-slate-800 space-y-4">
              ...
              >
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 block mb-1">{t('profile.old_password_label')}</label>
                  <input
                    type="password"
                    required
                    value={oldPassword}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 block mb-1">{t('profile.new_password_label')}</label>
                  <input
                    type="password"
                    required
                    value={newPassword}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 block mb-1">{t('profile.confirm_new_password_label')}</label>
                  <input
                    type="password"
                    required
            ...
            <div className="pt-6 border-t border-slate-800 space-y-3">
              <h4 className="font-bold text-white text-sm flex items-center gap-2">
                <Send className="w-4 h-4 text-cyan-400" />
                <span>{t('profile.support_help_title')}</span>
              </h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                ...
                >
                  ...
                  <div>
                    <span className="text-xs font-bold text-white block group-hover:text-cyan-300">{t('profile.telegram_support')}</span>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <span className="text-xs font-bold text-white block">{t('profile.email_support')}</span>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs font-bold text-white block">{t('profile.hotline')}</span>
