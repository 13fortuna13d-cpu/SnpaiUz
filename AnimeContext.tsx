...
import { Anime, Comment, AdConfig, Report, SocialSettings, Supporter, Category } from '../types';
...
interface AnimeContextType {
  ...
  categories: Category[];
  addCategory: (category: Partial<Category>) => { ok: boolean; error?: string };
...
export const AnimeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [categories, setCategories] = useState<Category[]>(() => {
    try {
      const saved = localStorage.getItem('senpaiuz_categories');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) return parsed;
      }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('senpaiuz_categories', JSON.stringify(categories));
    } catch (e) {
      console.error('Failed to save categories:', e);
    }
  }, [categories]);

  const addCategory = (data: Partial<Category>): { ok: boolean; error?: string } => {
    const id = (data.id || data.nameUz || '').trim();
    ...
    const newCategory: Category = {
      id,
      nameUz: data.nameUz || id,
      icon: data.icon || '🏷️',
      iconEmoji: data.iconEmoji,
      bg: data.bg || 'from-slate-700/20 to-slate-800/20',
      border: data.border || 'border-slate-600/30'
    };
  ...
  const deleteCategory = (id: string): { ok: boolean; error?: string } => {
    const inUse = animeList.some(a => a.genres.includes(id));
