...
export const CoinTopupModal: React.FC<CoinTopupModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto p-4 bg-slate-950/80 backdrop-blur-md flex items-center justify-center min-h-screen animate-fadeIn">
      <div className="relative w-full max-w-xl bg-slate-900 border border-amber-500/30 rounded-3xl p-5 sm:p-6 shadow-2xl overflow-y-auto max-h-[90vh] my-auto">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{t('coin.unlock_desc')}</p>
            </div>
          </div>
        ...
        {isSuccess ? (
          <div className="py-10 text-center space-y-4 animate-scaleUp">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-white">{t('coin.purchase_success')}</h3>
          ...
          <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    ...
                    {pkg.bonusCoins ? (
                      ...
                      <span className="inline-block mt-0.5 text-[10px] text-slate-500">{t('coin.standard_badge')}</span>
                    )}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-2.5">{t('coin.choose_payment_system')}</label>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-between">
              <div>
                <p className="text-[11px] text-slate-400">{t('coin.total_coins_received')}</p>
              ...
              >
                {isProcessing ? (
                  <span>{t('coin.payment_processing')}</span>
                ) : (
                  <>
