...
async function startServer() {
  ...
  app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', brand: 'SnpaiUz', timestamp: new Date().toISOString() });
  });
  ...
  async function sendEskizSms(phone: string, code: string): Promise<boolean> {
    ...
    try {
      ...
      const message = `SenpaiUz tasdiqlash kodi: ${code}. Kodni hech kimga bermang!`;
  ...
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      const newUser: ServerUser = {
        id: userId,
        name: username.trim(),
        email: isEmail ? cleanTarget : `${cleanTarget.replace(/\D/g, '')}@phone.snpaiuz.uz`,
        ...
        notifications: [
          {
            ...
            title: '🎉 Xush Kelibsiz!',
            message: 'SenpaiUz platformasiga ro\'yxatdan o\'tganingiz bilan tabriklaymiz!',
            date: 'Hozir',
            read: false,
            type: 'info'
          }
  ...
  app.post('/api/ai-generate-synopsis', async (req, res) => {
    try {
      ...
      if (!apiKey) {
        return res.json({
          synopsis: {
            uz: `${animeTitle} — SenpaiUz AI tomonidan tavsiflangan qiziqarli anime sarguzashti.`,
  ...
  app.get('/robots.txt', (req, res) => {
    ...
    res.send(`User-agent: *
Allow: /
Sitemap: https://snpaiuz.com/sitemap.xml
`);
  });
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`SenpaiUz server running on http://0.0.0.0:${PORT}`);
  });
}
